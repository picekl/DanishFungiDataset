...
def load_metadata(config: dict) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    train_df = pd.read_csv(f"./metadata/DanishFungi2023-train_metadata-v0.1.csv")

    valid_df = pd.read_csv(f"./metadata/DanishFungi2023-val_metadata-v0.1.csv")
...
def evaluate(
    ...
    id2class = dict(zip(valid_df["class_id"], valid_df["species"]))
...
def train_clf(
    ...
    **kwargs,
    ...
    evaluate(model, validloader, path=config["exp_path"], device=device)
